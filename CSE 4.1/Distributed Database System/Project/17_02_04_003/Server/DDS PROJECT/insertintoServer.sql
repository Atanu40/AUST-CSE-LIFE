...
CREATE OR REPLACE PROCEDURE insertToServer(input_id IN list1.id%TYPE,input_name IN list1.name%TYPE,input_math IN list1.m_marks%TYPE,input_eng IN list1.e_marks%TYPE,input_res IN list1.sres%TYPE)
IS
flag int := 0;
...
BEGIN
 ...
 ELSE  
   insert into list1 values(input_id,input_name,input_math,input_eng,input_res);
   commit;  
...
CREATE OR REPLACE TRIGGER insertDataOnServer 
AFTER INSERT 
ON studentList1
DECLARE
BEGIN
